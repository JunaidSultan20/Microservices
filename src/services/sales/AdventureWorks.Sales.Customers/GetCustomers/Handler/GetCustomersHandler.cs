using AdventureWorks.Sales.Customers.GetCustomers.Request;
using AdventureWorks.Sales.Customers.GetCustomers.Response;

namespace AdventureWorks.Sales.Customers.GetCustomers.Handler;
...
public class GetCustomersHandler : IRequestHandler<GetCustomersRequest, GetCustomersResponse>
{
    ...
    public async Task<GetCustomersResponse> Handle(GetCustomersRequest request, CancellationToken cancellationToken)
    {
        ...
        var customerList = await _unitOfWork.Repository<Customer>()
                                            .GetAsync(request.Pagination.PageNumber,
                                                      request.Pagination.PageSize);

        var count = await _unitOfWork.Repository<Customer>().GetCountAsync(customer => customer.CustomerId > 0);
