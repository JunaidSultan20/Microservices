...
public class MessageProducerTest : MessageProducerTestData
{
    ...
    public async Task When_Message_Is_Produced_Expect_Message_Pushed_To_Queue(
        string queue, string exchangeName, string exchangeType, string routeKey)
    {
        // Arrange
        var producer = SetupMockProducer().Build();
